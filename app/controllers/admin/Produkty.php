...
class Produkty extends ProduktyApiController
{
  ...
  public function upravit($id)
  {
    if ($_POST) {
      ...
      $this->model->createEdit($id, $reason ?? "update", $reason_id, SDiff::getObjectDiff($old_product,$this->model->getProductById($id), False), $_POST['edit_comment']);
    ...
    $edit = new Edit();
    if($e = $edit->getObjectEdits("product", $id)) {
